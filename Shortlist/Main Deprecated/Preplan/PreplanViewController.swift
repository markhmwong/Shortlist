	...
	func postTask(taskName: String, category: String, priorityLevel: Int) {
		...
		if (priorityLimitState) {
			...
			for task in taskList {
				taskStruct.append(TaskStruct(date: task.createdAt! as Date,name: task.name!, complete: task.complete, priority: task.priority, category: task.category ?? "General", reminder: task.reminder! as Date, reminderState: task.reminderState, details: task.details ?? ""))
			}
