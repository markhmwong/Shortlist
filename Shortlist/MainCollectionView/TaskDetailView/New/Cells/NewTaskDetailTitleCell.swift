...
extension UICellConfigurationState {
	var newTaskDetailTitleItem: NewTaskDetailTitleItem? {
		set { self[.newTaskDetailTitleItem] = newValue }
		get { return self[.newTaskDetailTitleItem] as? NewTaskDetailTitleItem }
	}
}

fileprivate extension UIConfigurationStateCustomKey {
	static let newTaskDetailTitleItem = UIConfigurationStateCustomKey("com.whizbang.cell.newTask.title")
}

class NewTaskDetailTitleCell: BaseCell<NewTaskDetailTitleItem> {
	
	override var configurationState: UICellConfigurationState {
		var state = super.configurationState
		state.newTaskDetailTitleItem = self.item
		return state
	}
	...
	override func updateConfiguration(using state: UICellConfigurationState) {
		setupViewsIfNeeded()
		taskName.text = state.newTaskDetailTitleItem?.name
	}
}
