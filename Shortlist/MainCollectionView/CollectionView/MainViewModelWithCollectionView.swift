...
class MainViewModelWithCollectionView: NSObject {
	...
	private func configureCellRegistration() -> UICollectionView.CellRegistration<TaskCollectionViewCell, TaskItem> {
		let cellConfig = UICollectionView.CellRegistration<TaskCollectionViewCell, TaskItem> { (cell, indexPath, item) in
			// configure cell
			cell.configureCellItem(with: item)
...
//			let size = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1.0), heightDimension: .estimated(55))
//			let item = NSCollectionLayoutItem(layoutSize: size)
...
//			group.contentInsets = NSDirectionalEdgeInsets(top: 20, leading: 20, bottom: 0, trailing: 20)
	...
	func itemForSelection(indexPath: IndexPath) -> TaskItem {
		guard let item = diffableDataSource.itemIdentifier(for: indexPath) else {
			return TaskItem(title: "Unknown", notes: "Unknown", priority: .high, completionStatus: true, reminder: "Reminder")
		}
		return item
	}
	...
	private func prepareDataSource() {
		tempDataSource = [
			TaskItem(title: "First Task. Fire Merlin Engine at full capacity. Stress test #1", notes: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum fdsfds dfs fds f.", priority: .high, completionStatus: true, reminder: "Reminder"),
			TaskItem(title: "Second Task. A longer title to test the dynamic height of the Task Collection View Cell. Second Task. A longer title to test the dynamic height of the Task Collection View Cell. ", notes: "Note two", priority: .medium, completionStatus: false, reminder: "Reminder"),
			TaskItem(title: "Third Task", notes: "Note three", priority: .high, completionStatus: false, reminder: "Reminder"),
			TaskItem(title: "Fourth Task", notes: "Note four", priority: .low, completionStatus: true, reminder: "Reminder"),
		]
	}
}
