...
class KeyboardNavigationContainer: UIView {
	...
	private func setupView() {
		...
		completeButton.trailingAnchor.constraint(equalTo: self.centerXAnchor).isActive = true
...
/*
	...
	Should be one view model passed between the new task guide
...
/*
	...
	Stage 1 - Title and priority

*/
class TaskNameAndPriorityViewController: UIViewController, UITextFieldDelegate, CategoryInputViewProtocol {
	func addCategory() {
		// to do
	}
...
//	}()
	...
	private lazy var nextButtonContainer: KeyboardNavigationContainer = {
...
//		view.addSubview(priorityDescription)
		...
		nextButtonContainer.doneClosure = {
...
//		priorityDescription.leadingAnchor.constraint(equalTo: nameInput.leadingAnchor).isActive = true
		...
		nextButtonContainer.anchorView(top: nil, bottom: nil, leading: view.leadingAnchor, trailing: view.trailingAnchor, centerY: nil, centerX: nil, padding: .zero, size: CGSize(width: 0.0, height: 50.0))
		...
		if let info = notification?.userInfo {
			...
			if let kbFrame = info[frameEndUserInfoKey] as? CGRect {
				...
				if (isKeyboardShowing) {
					nextButtonContainer.alpha = 1.0
					bottomConstraint?.constant = -kbFrame.height
				} else {
					nextButtonContainer.alpha = 0.0
